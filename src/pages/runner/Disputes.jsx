      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 marketSelection space-y-4">
        ...
        {hasActiveDispute && !showForm && (
          <div className={`rounded-3xl p-5 border border-yellow-500/20 bg-yellow-500/5`}>
            ...
            <div className={`mt-3 pt-3 border-t border-yellow-500/10 space-y-1`}>
              <p className="text-[10px] text-gray-400 uppercase tracking-widest font-bold">Reason</p>
              <p className={`text-xs font-semibold ${heading}`}>{getReasonLabel(currentOrderDispute.reason)}</p>
              {currentOrderDispute.description && (
                <p className="text-xs text-gray-400 mt-1">{currentOrderDispute.description}</p>
              )}
            </div>
          </div>
        )}
        ...
        {hasResolvedDispute && !showForm && (
          <div className={`rounded-3xl p-5 border ${
            ...
            <div className="space-y-1 mt-3">
              ...
              <p className={`text-xs font-semibold ${heading}`}>{getReasonLabel(currentOrderDispute.reason)}</p>
              {currentOrderDispute.description && (
                <p className="text-xs text-gray-400">{currentOrderDispute.description}</p>
              )}
            </div>
            ...
            {hasActiveOrder && (
              <button
                ...
                className="mt-4 w-full py-3 rounded-2xl text-[11px] font-black uppercase tracking-widest border border-dashed border-white/20 text-gray-400 hover:text-white hover:border-white/40 transition-all"
              >
                + Raise New Dispute
              </button>
            )}
        ...
        {!loading && !disputeError && disputes.length > 0 && (
          <div className="space-y-3">
            <p className="text-[10px] text-gray-400 uppercase tracking-widest font-bold">All Disputes</p>
            {disputes.map(d => (
              <div key={d._id} className={`rounded-3xl p-6 border ${card} space-y-3`}>
                ...
                <div className="flex items-center justify-between">
                  <p className="text-[10px] text-gray-400">Order: {d.orderId?.orderId || d.orderId}</p>
                  <p className="text-[10px] text-gray-400">
                ...
                {d.resolution && (
                  <div className={`rounded-2xl p-3 ${darkMode ? "bg-black-200" : "bg-gray-50"}`}>
                    <p className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">Resolution</p>
