...
export const Wallet = ({ darkMode, onBack, runnerId }) => {
  ...
  useEffect(() => {
    ...
    }, 500); // 500ms debounce so it doesn't fire on every keystroke
    ...
    return (
      <div className={`min-h-screen flex flex-col ${dark ? 'bg-black-100' : 'bg-white'}`}>
        <div className={`flex items-center gap-3 px-4 py-4 border-b ${dark ? 'border-black-200' : 'border-gray-100'}`}>
          ...
          <h1 className={`text-lg font-bold ${dark ? 'text-white' : 'text-black-200'}`}>Wallet</h1>
        </div>
  ...
  return (
    <div className={`min-h-screen flex flex-col ${dark ? 'bg-black-100' : 'bg-white'}`}>
      ...
      <div className={`flex items-center gap-3 px-4 py-4 border-b ${dark ? 'border-black-200' : 'border-gray-1001'
        ...
        <button
          onClick={onBack}
          className={`p-2 rounded-full transition-colors ${dark ? 'hover:bg-black-200' : 'hover:bg-gray-1001'
            }`}
        >
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <button
                ...
                className={`flex flex-col items-center gap-2 p-4 rounded-2xl ${dark ? 'bg-black-200' : 'bg-gray-1001'
                  }`}
              >
                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${dark ? 'bg-black-100' : 'bg-white'
                  }`}>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {wallet.transactions.slice(0, 3).map((txn, i) => (
                    <div key={i} className={`flex items-center gap-3 p-3 rounded-xl ${dark ? 'bg-black-200' : 'bg-gray-1001'
                      }`}>
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    disabled={isWithdrawing}
                    className={`flex-1 py-3 rounded-xl font-semibold bg-primary text-white ${isWithdrawing ? 'opacity-50' : 'hover:opacity-90'
                      }`}
                  >
                    {isWithdrawing ? (
            ...
            {withdrawStep === 'form' && (
              <>
                ...
                <div>
                  ...
                  <div className="relative">
                    <button
                      ...
                      className={`w-full p-4 rounded-xl border text-left flex items-center justify-between ${dark
                    ...
                    {showBankList && (
                      <div className={`absolute top-full left-0 right-0 z-50 mt-1 rounded-xl border max-h-64 overflow-hidden ${dark ? 'bg-black-100 border-black-200' : 'bg-white border-gray-1001'
                        } shadow-xl`}>
                ...
                <div>
                  ...
                  {/* Auto-verified account name appears here */}
                  {verifiedAccount && (
                    <div className="flex items-center gap-2 p-3 mt-2 rounded-xl bg-green-500/10">
                ...
                <button
                  ...
                  className={`w-full py-4 rounded-xl font-semibold text-white bg-primary transition-all ${!withdrawAmount ||
                    parseFloat(withdrawAmount) < 100 ||
                    parseFloat(withdrawAmount) > wallet.balance ||
                    !verifiedAccount
            ...
            {loading && wallet.transactions?.length === 0 ? (
              ...
              <>
                {wallet.transactions.map((txn, i) => (
                  <div key={i} className={`flex items-center gap-3 p-4 rounded-xl ${dark ? 'bg-black-200' : 'bg-gray-1001'
                    }`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <p className={`text-sm font-medium truncate ${dark ? 'text-white' : 'text-black-200'
                        }`}>
                        {getTransactionLabel(txn)}
                      </p>
                      <p className={`text-xs ${dark ? 'text-gray-1002' : 'text-gray-600'}`}>
                ...
                {wallet.pagination?.pages > page && (
                  <button
                    ...
                    disabled={loading}
                    className={`w-full py-3 rounded-xl text-sm font-medium ${dark ? 'bg-black-200 text-gray-1002' : 'bg-gray-1001 text-black-200'
                      }`}
                  >
                    {loading ? (
                      <span className="flex items-center justify-center gap-2">
