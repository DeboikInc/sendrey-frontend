...
const ItemSubmissionMessage = ({ message, darkMode, onApprove, onReject }) => {
  ...
  if (status === 'approved') {
    return (
      <div className="flex justify-center my-4 px-4">
        <div className={`max-w-md w-full rounded-2xl shadow-lg border ${darkMode ? 'bg-green-900/20 border-green-800' : 'bg-green-50 border-green-200'
          } p-6`}>
  ...
  if (status === 'rejected') {
    return (
      <div className="flex justify-center my-4 px-4">
        <div className={`max-w-md w-full rounded-2xl shadow-lg border ${darkMode ? 'bg-red-900/20 border-red-800' : 'bg-red-50 border-red-200'
          } p-6`}>
  ...
  return (
    <>
      <div className="flex justify-center my-4 px-4">
        <div className={`max-w-md w-full rounded-2xl shadow-lg ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'
          } border p-6`}>
          ...
          <div className="space-y-3 mb-4">
            {items.map((item, index) => (
              <div key={index} className={`flex gap-3 p-3 rounded-lg ${darkMode ? 'bg-gray-700/50' : 'bg-gray-50'
                }`}>
                {item.photoUrl && (
                  <img
                    src={item.photoUrl}
          ...
          {/* Total */}
          <div className={`flex justify-between items-center p-3 rounded-lg mb-4 ${darkMode ? 'bg-gray-700' : 'bg-gray-100'
            }`}>
            <span className={`font-semibold ${darkMode ? 'text-white' : 'text-black-100'}`}>
              Total Amount
            </span>
          ...
          <div className="flex gap-3">
            <button
              ...
              className={`flex-1 py-3 rounded-xl font-semibold transition-all flex items-center justify-center gap-2 ${darkMode
                ? 'bg-secondary hover:bg-secondary/80 text-white'
                : 'bg-secondary hover:bg-secondary/80 text-white'
                } ${isProcessing || submitted ? 'bg-gray-500 cursor-not-allowed text-gray-300' : ''}`}
            >
              <X className="w-5 h-5" />
              Reject
            </button>
            <button
              ...
              className={`flex-1 py-3 rounded-xl font-semibold transition-all flex items-center justify-center gap-2 ${darkMode
                ...
                : 'bg-primary hover:bg-primary/80 text-white'
                } ${isProcessing || submitted ? 'opacity-50 cursor-not-allowed bg-gray-500 text-gray-300' : ''}`}
            >
          ...
          <div className={`w-full max-w-md rounded-2xl shadow-2xl p-6 ${darkMode ? 'bg-gray-900 border border-gray-800' : 'bg-white'
            ...
            <div className="flex gap-3 mt-4">
              <button
                ...
                className={`flex-1 py-2 rounded-lg font-medium ${darkMode
              ...
              <button
                ...
                className={`flex-1 py-2 rounded-lg font-medium ${darkMode
                  ? 'bg-secondary hover:bg-black-100 text-white'
                  : 'bg-secondary hover:bg-black-100 text-white'
                  } ${(!rejectReason.trim() || isProcessing || submitted) ? 'bg-gray-500 text-gray-300 cursor-not-allowed' : ''}`}
              >
