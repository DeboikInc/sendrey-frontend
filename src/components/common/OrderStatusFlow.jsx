...
const OrderStatusFlow = ({ isOpen, onClose, orderData, darkMode }) => {
    ...
    const handleStatusClick = (statusKey) => {
        if (completedStatuses.includes(statusKey)) {
            setCompletedStatuses(completedStatuses.filter(s => s !== statusKey));
        } else {
            setCompletedStatuses([...completedStatuses, statusKey]);
        }
    };
